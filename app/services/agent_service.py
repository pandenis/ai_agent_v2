...
from app.core.agent_config import TaskType, agent_registry
...
class AgentService:
    ...
    async def generate_response(
        ...
        if not is_valid:
            return {"response": f"Invalid input: {error}", "status": "error", "agent": "security_validator", "error": error}
        ...
        if not is_healthy:
            return await self._fallback_response(prompt, agent_name, reason="Agent health check failed")
    ...
    async def select_best_agent_for_task(self, prompt: str, task_type: Optional[TaskType] = None) -> str:
        """
    ...
    async def _fallback_response(self, prompt: str, failed_agent: str, reason: str = "Agent not available") -> Dict[str, Any]:
        """
    ...
    async def get_agent_status(self) -> Dict[str, Any]:
        ...
        for agent_name in agent_registry.list_agents(enabled_only=False):
            ...
            status["agents"][agent_name] = {
                ...
                "capabilities": [{"name": c.name, "confidence": c.confidence} for c in config.capabilities],
            }
