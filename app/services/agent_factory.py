...
class OllamaAgent(BaseAgent):
    ...
    async def generate(
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "response": f"Error from {self.name}: {str(e)}"}
...
class CloudAgent(BaseAgent):
    ...
    async def generate(
        ...
        if not self.api_key:
            return {"status": "error", "error": "API key not configured", "response": f"{self.name} requires API key"}
    ...
    async def health_check(self) -> bool:
        ...
        try:
            response = await self.client.get(
                f"{self.api_base_url}/models", headers={"Authorization": f"Bearer {self.api_key}"}, timeout=5.0
            )
