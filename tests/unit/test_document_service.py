...
async def test_add_and_search_documents():
    ...
    await service.add_document(
        doc_id="doc1", text="Python is a programming language used for AI and web development", metadata={"type": "tutorial"}
    )

    await service.add_document(
        doc_id="doc2", text="JavaScript is used for web frontend development", metadata={"type": "tutorial"}
    )
...
async def test_document_count():
    ...
    await service.add_document(doc_id="test-count", text="Test document for counting", metadata={"source": "test"})
...
async def test_delete_document():
    ...
    await service.add_document(doc_id=doc_id, text="Document to be deleted", metadata={"source": "test"})
...
async def test_add_document_without_metadata():
    ...
    await service.add_document(doc_id=doc_id, text="Document without metadata")
